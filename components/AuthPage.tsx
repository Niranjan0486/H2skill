...
const AuthPage: React.FC<AuthPageProps> = ({ onLoginSuccess }) => {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    // NOTE: In production, integrate Firebase Auth here
    // await signInWithEmailAndPassword(auth, email, password);
    
    try {
      const user = await mockAuth(email || 'demo@ecoverify.com');
      onLoginSuccess(user);
    } catch (error) {
      console.error(error);
    } finally {
    ...
    <div className="min-h-screen bg-slate-50 flex flex-col justify-center items-center px-4">
      <div className="bg-white w-full max-w-md p-8 rounded-2xl shadow-xl border border-slate-100">
        ...
        <div className="mt-6">
          ...
          <button 
            type="button"
            className="mt-4 w-full bg-white border border-slate-200 text-slate-700 py-2.5 rounded-lg font-medium hover:bg-slate-50 transition-colors flex items-center justify-center gap-2"
          >
